...
set_parameters($sel, { "Grupy zabezpieczeń" => {"querysharegroup" => {type => "select", value => "canconfirm"}} });
...
ok($text =~ /Masz nowe wyszukiwanie o nazwie Shared Selenium buglist./, "New search named 'Shared Selenium buglist' has been created");
...
$sel->wait_for_page_to_load_ok(WAIT_TIME);
$sel->title_is("Lista błędów: Shared Selenium buglist");
# You cannot delete other users' Wyszukiwania.
ok(!$sel->is_text_present("Usuń wyszukiwanie „Shared Selenium buglist”"), "'Forget...' link not available");
...
ok($text =~ /Masz nowe wyszukiwanie o nazwie helpwanted./, "New search named helpwanted has been created");
...
# Our own Wyszukiwania are displayed in the footer by default.
...
$sel->wait_for_page_to_load_ok(WAIT_TIME);
$sel->title_is("Lista błędów: helpwanted");
...
$text = trim($sel->get_text("message"));
ok($text =~ /Wyszukiwanie Shared Selenium buglist zostało usunięte./, "The 'Shared Selenium buglist' search is gone");
logout($sel);
...
$sel->title_is("Usuwanie wyszukiwania");
$text = trim($sel->get_text("message"));
ok($text =~ /Wyszukiwanie helpwanted zostało usunięte./, "The 'helpwanted' search is gone");
logout($sel);
