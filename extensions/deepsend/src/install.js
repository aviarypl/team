...
var XpiInstaller = {
	...
	extFullName: 'Deepest Sender', // The name displayed to the user (don't include the version)
	extShortName: 'deepestsender', // The leafname of the JAR file (without the .jar part)
	extVersion: '0.8.0',
	extAuthor: 'David Murray',
	extLocaleNames: ['en-AU', 'it-IT', 'ru-RU', 'es-ES','pl-PL'], // e.g. ['en-US', 'en-GB']
	...
	install: function()
	{
		...
		else if (!this.silentInstall)
		{
			...
			this.profileInstall = Install.confirm('Install ' + this.extFullName + ' ' + this.extVersion + ' to your Profile directory (OK) or your Browser directory (Cancel)? You will need write permissions to the application directory, no matter what method you choose.');
		}
		...
		Install.addFile(null, "chrome/icons/default/deepestsender.ico", iconfolder, null);
		Install.addFile(null, "chrome/icons/default/deepestsender.xpm", iconfolder, null);
		var prefsfolder = getFolder(getFolder("Program", "defaults"), "pref");
		Install.addFile(null, "defaults/preferences/deepestsender.js", prefsfolder, null);
